...
public:
	...
	void deleteNodeNeighbor(vector<Vertex>::iterator pvertex)
	{
		...
		#pragma omp parallel for default(shared)
		for(int neighbor_edge=0;neighbor_edge<(pvertex->edges).size();neighbor_edge++)
		{
			if(pvertex->edges[neighbor_edge]==1)
			{
				deleteNode(neighbor_edge);
			}
		}
		//cout<<pvertex->deleted<<endl;
		pvertex->num_edges=0;
	}
	void deleteNode(int nvertex)
	{
		...
		#pragma omp critical
		{
			num_vertices-=(1-(vertices[nvertex]).deleted);
	...
	void deleteSet(vector<int>::iterator begin,int size)
	{
		...
		rep(i,0,size,1)
		{
			deleteNodeNeighbor(vertices.begin()+*(begin+i));
		}
		#pragma omp parallel for default(shared)
		rep(i,0,vertices.size(),1)
		{
...
int main(int argc, char** argv)
{
	...
	srand(time(NULL));
	int num_round=100;
	for(int n_vertices=100;n_vertices<=1000;n_vertices+=100){
	    ...
	    fprintf(ftime,"%d\t%.2fs\n",n_vertices,(double)(clock() - tStart)/CLOCKS_PER_SEC);
